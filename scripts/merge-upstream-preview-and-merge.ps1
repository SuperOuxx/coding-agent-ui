...
try {
  ...
  if (-not (Confirm-Step -Prompt "Continue to real merge stage?" -DefaultYes $true)) {
  ...
  if ([string]::IsNullOrWhiteSpace($MergeBranch)) {
    $MergeBranch = "merge/upstream-preview-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss")
  }
  ...
  Write-Section "Step 4: Merge --no-commit (human-in-the-loop)"
  Invoke-Git merge --no-commit --no-ff $upstreamRef | Out-Null
  if ($LASTEXITCODE -ne 0) {
    $conflicts = (& git diff --name-only --diff-filter=U)
    if (-not $conflicts) {
      throw "Merge failed without conflict list. Inspect git output manually."
    }
    Write-Host "Conflicts detected. Entering interactive resolver."
  ...
  Write-Host ""
  Write-Host "Merge changes are staged but not committed."
  & git status --short --branch

  if (-not (Confirm-Step -Prompt "Create merge commit now?" -DefaultYes $true)) {
    Write-Host "Stop here. Review with 'git diff --cached' and commit manually."
    exit 0
  }

  $defaultMessage = "merge: sync $upstreamRef into $MainBranch"
